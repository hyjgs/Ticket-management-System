...
public class UserAndRolesManagementadd {
	...
	public UserAndRolesManagementadd(){
		action=new Action();
		frame=new JFrame("添加角色");
		title=new JLabel("角色添加");
		...
		ok.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(nameField.getText().trim().equals("")){
					JOptionPane.showMessageDialog(null, "角色名不能为空");
					return;
				}
...
//				System.out.println("i==="+i);
				...
				role.setName(name);
				Request req=new Request(Request.ADD_ROLE_REQUEST);
				req.addData("newrole", role);
//				req.addData("角色名", nameField.getText().trim());
//				req.addData("权限编号",i+"");
				Response res=action.doAction(req);
				boolean flag=(Boolean) res.getData("添加状态");
				if(flag){
