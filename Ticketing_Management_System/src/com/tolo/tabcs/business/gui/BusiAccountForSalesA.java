...
public class BusiAccountForSalesA extends JPanel{
	...
	public static String applyTime;
	private int m;
	 ...
	 * 初始化各种标签，按钮
	 * */
	public BusiAccountForSalesA(){
		title=new JLabel("未结算的营业记录：");
...
//		JScrollPane滚动面板和表的初始化，并将表添加到面板中。
		...
		table.getColumn(table.getColumnName(0)).setMaxWidth(20);
...
//		table.getColumn(table.getColumnName(4)).setMaxWidth(100);
		...
		message=new JLabel("共选中0笔营业记录，其中订票0，退票0，改签0；合计金额：0元");
		message.setForeground(Color.RED);
		//JButton的初始化
		settleAccountsButton=new JButton("申请结算>>");
	...
	private void addEventListener(){
		...
		refresh.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				...
				Request req = new Request(Request.SEARCH_UNCHECKED_SALES_RECORD_REQUEST);
				...
				if (!(records.length==0)) {
					...
					JOptionPane.showMessageDialog(BusiAccountForSalesA.this, "没有未结算的记录");
				}
				int buy_num=0;//购票数量
				int back_num=0;//退票数量
				int update_num=0;//改签数量	
				double money=0;//总金额；
				System.out.println("the id is "+req.getData("user_id"));
				for(int i=0;i<checkBoxs.length;i++){
					checkBoxs[i].setSelected(false);
				}
				table.repaint();
				message.setText("共选中0笔营业记录，其中订票0，退票0，改签0；合计金额：0元");
			}
		//	message=new JLabel();
		});
		...
		settleAccountsButton.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {			
				Action action = new Action();
				Request req = new Request(Request.APPLY_UNCHECKED_SALES_RECORD_REQUEST);
				req.addData("user_id",((User)LoginFrame.onlineUsers.get("user")).getId());
				...
				if(flag){
					JOptionPane.showMessageDialog(BusiAccountForSalesA.this,	"申请一级结算成功，请等待结果");
				}else {
					JOptionPane.showMessageDialog(BusiAccountForSalesA.this,	"申请一级结算失败，请重试");
				}
				for (int i = 0; i < 50; i++) {
					for (int j = 1; j < 5; j++) {
						table.setValueAt("", i, j);
	...
	private void showMessage(){
		...
		for(int i=0;i<checkBoxs.length;i++){
			if(checkBoxs[i].isSelected()&&table.getValueAt(i, 1)!=""){
				allMoney+=Integer.parseInt((String) table.getValueAt(i, 4));
				num++;
				if(table.getValueAt(i, 3).equals("购票")){
					num1++;
		...
		message.setText("共选中" + num+"笔营业记录，其中购票" + num1+",退票"+ num2+",改签" + num3+"；合计金额：" +allMoney+"元");
	}
